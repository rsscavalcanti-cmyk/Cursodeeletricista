...
const CourseApp = (() => {
  ...
  function _load() {
    ...
    } catch (e) { _state = _default(); }
  }
  function _default() {
    return { version: 1, studentName: '', cpf: '', startDate: null, completionDate: null, courseComplete: false, certificateId: null, lessons: {}, quizScores: {} };
  }
  function _save() {
  ...
  function _checkComplete() {
    ...
    const passed = Object.keys(_state.quizScores).filter(k => _state.quizScores[k].passed).length;
  ...
  function getProgress() {
    ...
    return {
      studentName: _state.studentName, cpf: _state.cpf, startDate: _state.startDate,
      completionDate: _state.completionDate, courseComplete: _state.courseComplete,
      certificateId: _state.certificateId, lessons: _state.lessons, quizScores: _state.quizScores,
      totalDone: done, totalLessons: total,
      ...
      isLessonDone(modId, lessonId) { return !!_state.lessons[`${modId}-${lessonId}`]?.done; },
      isQuizPassed(modId) { return !!_state.quizScores[modId]?.passed; }
    };
  }
  ...
  function saveQuiz(modId, score, total) {
    ...
    _state.lessons[`${modId}-quiz`] = { done: passed, score, total, at: new Date().toISOString() };
    _checkComplete();
    _save();
    return passed;
  }
  function reset() {
    const name = _state?.studentName || ''; const cpf = _state?.cpf || '';
    _state = _default(); _state.studentName = name; _state.cpf = cpf;
  ...
  function validateCPF(cpf) {
    ...
    let r = (sum * 10) % 11; if (r === 10 || r === 11) r = 0; if (r !== +d[9]) return false;
    sum = 0;
  ...
  _load();
  return { init, getUser, setUser, hasUser, getProgress, markLesson, saveQuiz, reset, formatCPF, validateCPF };
})();