...
(function () {
  ...
  const m1l1 = lesson(1,1); if (m1l1) {
    ...
    m1l1.content += '<div class="alert alert-warning"><strong>Limite deste curso livre:</strong> O registro de conclusão é introdutório; não habilita atuação profissional, não concede autorização NR-10 e não substitui treinamento oficial, supervisão ou responsabilidade técnica.</div>';
  }
  ...
  COURSE_DATA.modules.flatMap(m => m.lessons).forEach(l => {
  ...
  document.addEventListener('click', function (event) {
    const link = event.target.closest('a[href="certificado.html"]');
    if (link) { event.preventDefault(); window.location.href = 'encerramento.html'; }
  }, true);
})();
